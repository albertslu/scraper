  ...
  try {
    ...
    try {
      const categoryLinks = await page.$$eval('a', links => 
        links.map(link => ({ href: link.href, text: link.textContent?.toLowerCase() || '' }))
          .filter(link => 
    ...
    try {
      ...
      if (searchInput) {
        ...
        const paginationLinks = await page.$$eval('a[href*="page"], a[href*="next"]', links => 
          links.map(link => link.href).filter(href => href.includes('manta.com'))
        );
