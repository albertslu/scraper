    ...
    try {
        ...
        try {
            ...
            const categoryLinks = await page.$$eval('a', links => links.filter(link => link.textContent?.toLowerCase().includes('restaurant') ||
                ...
                link.href.includes('restaurant')).map(link => ({ href: link.href, text: link.textContent })));
        ...
        const businessLinks = await page.$$eval('a', links => links.filter(link => link.href.includes('manta.com') &&
            (link.href.includes('/c/') || link.href.includes('/business/') || link.href.includes('/company/'))).map(link => link.href));
