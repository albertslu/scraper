...
async function main() {
    ...
    try {
        ...
        for (const selector of listingSelectors) {
            try {
                const links = await page.$$eval(selector, (elements) => elements.slice(0, 3).map(el => el.href).filter(href => href && href.includes('manta.com')));
                if (links.length > 0) {
                    allUrls.push(...links);
        ...
        if (allUrls.length === 0) {
            ...
            try {
                const genericLinks = await page.$$eval('a', (elements) => elements.slice(0, 5)
                    .map(el => el.href)
