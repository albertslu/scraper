    ...
    if (rfsElements.length === 0) {
      ...
      for (let i = 0; i < headings.length; i++) {
        ...
        try {
          ...
          if (!headingText || headingText.trim().length === 0) continue;
      ...
      for (let i = 0; i < rfsElements.length; i++) {
        ...
        try {
          ...
          if (!elementText || elementText.trim().length < 50) continue;
            ...
            if (lines.length >= 2) {
              ...
              if (startup_idea.length > 15 && description.length > 80) {
                ...
                if (validation.success) {
                  ...
                  if (results.length >= 20) break; // Limit fallback results
                }
              }
            }
          }
...
function inferCategory(idea: string, description: string): string {
