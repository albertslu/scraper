...
async function main() {
    ...
    try {
        ...
        if (!listingElements || listingElements.length === 0) {
            ...
            for (let i = 0; i < Math.min(allElements.length, 20); i++) {
                try {
                    ...
                    if (text && (text.toLowerCase().includes('restaurant') ||
                        ...
                        const nameElement = allElements[i].locator('h1, h2, h3, h4, .name, .title, .business-name').first();
                        const addressElement = allElements[i].locator('.address, .location, [class*="address"]').first();
                        let name = '';
                        let address = '';
            ...
            for (let i = 0; i < testLimit; i++) {
                try {
                    ...
                    const nameSelectors = ['h1', 'h2', 'h3', 'h4', '.name', '.title', '.business-name', 'a[href*="business"]'];
                    let name = '';
                    ...
                    const addressSelectors = ['.address', '.location', '[class*="address"]', '.street', '.addr'];
                    let address = '';
                    ...
                    // If no specific address selector found, try to extract from full text
                    if (!address && name) {
                        const fullText = await listing.textContent();
                        if (fullText) {
                            // Look for address patterns in the text
                            const addressMatch = fullText.match(/\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Way|Circle|Cir|Court|Ct|Place|Pl)/i);
                            if (addressMatch) {
                    ...
                    if (name && address) {
                        ...
                        console.log(`⚠️ Missing data for listing ${i + 1}: name="${name}", address="${address}"`);
                    }
                }
                catch (error) {
        ...
        if (results.length === 0) {
            console.log('⚠️ No valid restaurant data found. The site structure may have changed or requires different navigation.');
            console.log('💡 Suggestion: Try navigating to a specific restaurant category page first.');
        }
